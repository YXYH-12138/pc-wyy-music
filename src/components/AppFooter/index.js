import React, { memo } from 'react'
import { footerLinks, footerImages } from '@/common/local-data'
import { FooterWrapper, FooterLeft, FooterRight } from './style'

export default memo(function AppFooter() {
  return (
    <FooterWrapper>
      <div className="wrap-v2 content">
        <FooterLeft>
          <p>
            {
              footerLinks.map((item) => (
                <span key={item.title}>
                  <a href={item.link} target="_block">{item.title}</a>
                  <span className="line">|</span>
                </span>
              ))
            }
          </p>
          <p>
            <span className="sep">网易公司版权所有©1997-2020</span>
            <span>杭州乐读科技有限公司运营：</span>
            <a href="https://p1.music.126.net/Mos9LTpl6kYt6YTutA6gjg==/109951164248627501.png" rel="noopener noreferrer" target="_blank">浙网文[2018]3506-263号</a>
          </p>
          <p>
            <span className="sep">违法和不良信息举报电话：0571-89853516</span>
            <span>举报邮箱：</span>
            <a href="mailto:ncm5990@163.com" className="email">ncm5990@163.com</a>
          </p>
          <p>
            <span className="sep">粤B2-20090191-18</span>
            <a className="sep" href="http://www.beian.miit.gov.cn/publish/query/indexFirst.action" rel="noopener noreferrer" target="_blank">工业和信息化部备案管理系统网站</a>
            <a className="police" target="_block" href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=33010902002564">
              <span className="police-logo"></span>
              <span>浙公网安备 33010902002564号</span>
            </a>
          </p>
        </FooterLeft>
        <FooterRight className="right">
          {
            footerImages.map((item) => {
              return (
                <li className="item" key={item.link}>
                  <a className="link" href={item.link} rel="noopener noreferrer" target="_blank"> </a>
                  <span className="title">{item.title}</span>
                </li>
              )
            })
          }
        </FooterRight>
      </div>
    </FooterWrapper>
  )
})
